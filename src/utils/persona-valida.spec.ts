...
describe('Color_pelo', () => {
  it('Deberia validar el campo color_pelo vacio', async () => {
    const { color_pelo, ...body } = requestBody;
    const err = await validatorDto(CrearPersonaDto, body);
    expect(err.mensaje).toContain('Error de validacion');
    expect(err.problema).toContain('color_pelo es requerido');
  });
  it('Deberia validar el campo color_pelo sea string', async () => {
    const body = { ...requestBody };
    body.color_pelo = 32 as any;
    const err = await validatorDto(CrearPersonaDto, body);
    expect(err.mensaje).toContain('Error de validacion');
    expect(err.problema).toContain('color_pelo debe ser string');
  });
});
describe('Color_piel', () => {
  it('Deberia validar el campo color_piel vacio', async () => {
    const { color_piel, ...body } = requestBody;
    const err = await validatorDto(CrearPersonaDto, body);
    expect(err.mensaje).toContain('Error de validacion');
    expect(err.problema).toContain('color_piel es requerido');
  });
  it('Deberia validar el campo color_piel sea string', async () => {
    const body = { ...requestBody };
    body.color_piel = 32 as any;
    const err = await validatorDto(CrearPersonaDto, body);
    expect(err.mensaje).toContain('Error de validacion');
    expect(err.problema).toContain('color_piel debe ser string');
  });
});
